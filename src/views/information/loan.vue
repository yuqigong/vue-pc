        ...
        <el-row>
            <el-form :inline="true" ref="searchDate" :model="searchDate" label-width="110px">
                ...
                <el-form-item
                    ...
                    prop="ln_no"
                >
                    <el-input :maxlength="50" v-model="searchDate.ln_no" placeholder="请输入内容" auto-complete="off"
                              icon="search"/>
                </el-form-item>
                <el-form-item
                    label="源系统代码:"
        ...
        <el-row>
            <el-table
                ...
                <el-table-column
                    ...
                    width="240"
                    align="center"
                    class-name="cc-action"
                    label="操作">
                    <template scope="scope">
                        <el-button @click="queryRefund(scope.$index, scope.row)" type="text" size="small">查询还款信息
                        </el-button>
                        <el-button @click="queryRepayment(scope.$index, scope.row)" type="text" size="small">查询还款计划
                        </el-button>
                        <el-button @click="queryLoanstatus(scope.$index, scope.row)" type="text" size="small">查询贷款状态
                        </el-button>
                        <el-button @click="queryInterest(scope.$index, scope.row)" type="text" size="small">查询日息即提
                        </el-button>
                    </template>
                </el-table-column>
            </el-table>
            <p v-if="totalAmount">{{totalAmount.drawdownAmtTxt}}: {{totalAmount.drawdownAmt}}</p>
...
<script lang="babel">
    import {getLoanList} from '../../api/information'
    ...
    export default {
        ...
        created () {
            if (this.$route.query.custId) {
                this.queryLoanList({
                    cust_id: this.$route.query.custId,
                    page_no: this.paging.currentPage,
                    page_size: this.paging.pageSize
                });
                this.searchDate.cust_id = this.$route.query.custId;
        ...
        methods: {
            queryLoanList (params) {
                ...
                getLoanList(params).then(response => {
            ...
            queryRefund (index, row) {
                console.log(row);
                this.$router.push({path: '/information/refund', query: {lnNo: row.lnNo}})
            },
            queryRepayment (index, row) {
                console.log(row);
                this.$router.push({path: '/information/repayment', query: {lnNo: row.lnNo}})
            },
            queryLoanstatus (index, row) {
                console.log(row);
                this.$router.push({path: '/information/loanstatus', query: {lnNo: row.lnNo}})
            },
            queryInterest (index, row) {
                console.log(row);
                this.$router.push({path: '/information/interest', query: {lnNo: row.lnNo}})
            },
